...
bson_filename='/hme/jagadish/edx/forums_dump_20131018/app15682028/contents.bson'
if(len(sys.argv)!=2):
...
try:
  ...
  cur.execute('truncate table contents');
...
def getData():
 try:
   cur.execute("select body from contents");
...
def populateUserCache () : 
  ...
  try:
    ...
    cur.execute("select user_int_id,name,screen_name,anon_screen_name from UserGrade");
    for c in cur.fetchall() :
      obj=[];
      obj.append(c[1]);
      obj.append(c[2]);
...
#     print 'NEW COMBINED BODY AFTER NAME STRIPING %s \n'%(body);
  try:
    cur.execute("insert into contents(type,anonymous,anonymous_to_peers,at_position_list,user_int_id,body,course_display_name,created_at,votes,count,down_count,up_count,up,down) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(_type,anonymous,anonymous_to_peers,at_position_list,author_id,body,course_id,created_at,votes,count,down_count,up_count,up,down));
...
for c in collection.find():
  ...
  try:
    ...
    logger.info('error in conveersion')
    up='-1'
...
def generateInsertQuery(collectionobject)
  insertStatement='insert into contents(%s) values(%s)';
